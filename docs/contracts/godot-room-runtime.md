...
| `playerSpawn` `{x,y}` tile | Horizontal: center of column `x`. Vertical: **feet Y** = top of first **standable** solid at/under that column (solid with **air** in the cell above — skips ceiling mass and interior wall fill). Fallback: center of the spawn row if none found. |
